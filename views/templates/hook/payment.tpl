...
<div class="sofinco_error" style="margin-left:15px;">
...
<p class="payment_module sofinco_module">
	<img src="{$sofincoImagePath}Paiement_3X.png" />
	{foreach from=$sofincoRecurring item=card name=cards}
		<a href="{$card.url|escape:'html'}&amp;recurring=1">
			<img src="{$card.image}" alt="{$card.card}" title="{$card.card}" />
		</a>
	{/foreach}
	{l s='Pay by' mod='sofinco'} {l s='card 3 times without fees' mod='sofinco'}
</p>
{/if}
