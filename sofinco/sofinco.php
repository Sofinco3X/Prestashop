...
class Sofinco extends PaymentModule
{
    private $_requestId;
    private $_config;
    private $_helper;
    ...
    public function __construct()
    {
        ...
        $this->_config = new SofincoConfig();
        $this->_helper = new SofincoHelper($this);
    ...
    public function getContent()
    {
        ...
        require_once(dirname(__FILE__) . '/classes/SofincoAdminConfig.php');

        $admin = new SofincoAdminConfig($this);
    ...
    public function hookAdminOrder($params)
    {
        ...
        $admin = new SofincoAdminOrder($this);

        $w = new SofincoHtmlWriter();
    ...
    public function hookCancelProduct($params)
    {
        ...
        if ($helper->canCapture($orderId)) {
            // Update database
            $sql = 'UPDATE `%ssofinco_order` SET `amount` = `amount` - %d'
                    . ' WHERE `id_order` = %d';
    ...
    public function hookPayment($params)
    {
        ...
        $smarty->assign('sofincoImagePath', $this->getImagePath());
        $smarty->assign('sofincoCards', $cards);
        $smarty->assign('sofincoProduction', $this->getConfig()->isProduction());
        ...
        if ($this->getConfig()->isRecurringEnabled()) {
            if ($cart->getOrderTotal() >= $this->getConfig()->getRecurringMinimalAmount()) {
                $smarty->assign('sofincoRecurring', $recurringCards);
            }
        }

        $reason = Tools::getValue('sofincoReason');
        if (($reason != 'cancel') && ($reason != 'error')) {
            $reason = null;
        }
        $smarty->assign('sofincoReason', $reason);
        ...
        if (version_compare(_PS_VERSION_, '1.6', '<')) {
            ...
            if (Configuration::get('PS_ORDER_PROCESS_TYPE') == 1 && Dispatcher::getInstance()->getController() == 'orderopc') {
                $sofincoCSS = Tools::file_get_contents($this->context->link->getMediaLink($cssPath));
                $sofincoCSS = '<style type="text/css">'.$sofincoCSS.'</style>';
            } else {
                $sofincoCSS = '';
                $this->context->controller->addCSS($cssPath, 'all');
            }
            $smarty->assign('sofincoCSS', $sofincoCSS);
    ...
    public function hookUpdateOrderStatus($params)
    {
        ...
        if (empty($details) || ($details['payment_by'] != 'Sofinco')) {
            return;
        }
    ...
    {
        $actionsBO = Configuration::get('SOFINCO_BO_ACTIONS', 0);
        ...
        if (false !== $orderId && 1 == $actionsBO) {
            ...
            // Payment must be "standard" method
            $details = $this->getHelper()->getOrderDetails($orderId);
            if (empty($details) || ($details['payment_by'] != 'Sofinco' && $details['payment_by'] != 'PayboxSystemRecurring')) {
                return;
            }
    ...
    {
        if (Tools::isSubmit('sofincoRefund')) {
    ...
    public function install()
    {
        ...
        $installer = new SofincoInstaller();
    ...
    {
        $this->logDebug(sprintf('(IPN) Cart %d: Standard IPN', $cart->id));
        ...
        $this->logDebug(sprintf('(IPN) Cart %d: Validating order', $cart->id));
        if (!$cart->OrderExists()) {
            try {
                ...
                $this->logFatal(sprintf('(IPN) Cart %d: Error validating PrestaShop order:', $cart->id, $e->getMessage()));
            }
            if (!$result) {
                $this->logFatal(sprintf('(IPN) Cart %d: Unable to validate PrestaShop order', $cart->id));
                throw new Exception('Unable to validate order');
            }
            $this->logDebug(sprintf('(IPN) Cart %d: Order %d validated, creating details', $cart->id, $this->currentOrder));
            $order = new Order($this->currentOrder);
        } else {
            $orderId = Order::getOrderByCartId((int)($cart->id));
            $order = new Order($orderId);
            $this->logDebug(sprintf('(IPN) Cart %d: got Order %d', $cart->id, $order->id));
            // Check if the order had this state
            if (empty($order->getHistory(false, $this->_config->getSuccessState()))) {
        ...
        $this->logDebug(sprintf('(IPN) Cart %d: Order %d / %s', $cart->id, $order->id, $message));
    }
    ...
    public function onMixedIPNSuccess(Cart $cart, array $params)
    {
        ...
        if (empty($orderId)) {
            ...
            $state = Configuration::get('SOFINCO_MIDDLE_STATE_NX');
    ...
    public function onThreetimeIPNSuccess(Cart $cart, array $params)
    {
        ...
        if (empty($orderId)) {
            ...
            // Save payment information
            $this->getHelper()->addOrderPayment($order, 'capture', $params, 'SofincoRecurring');
...
//            $this->getHelper()->addOrderNote($order, $message);
            ...
            switch ($details['number_term']) {
                ...
                case 1:
                    ...
                    $changeHistory = new OrderHistory();
                    $changeHistory->id_order = $order->id;
                    $changeHistory->changeIdOrderState(Configuration::get('SOFINCO_LAST_STATE_NX'), $changeHistory->id_order);
                    $changeHistory->addWithemail();
                    break;
