 ...
 */
class SofincoHelper extends SofincoAbstract
{
    ...
    private function _callDirect(Order $order, $type, $trxId, $callId, $amount, $cardType)
    {
        ...
        $message = 'Order %d: call Sofinco - DEVISE=%s MONTANT=%s NUMAPPEL=%s NUMTRANS=%s TYPE=%s';
        ...
        $this->logDebug(sprintf('Order %d: Sofinco error code %d', $order->id, $result['code']));
        $this->logDebug(sprintf('Order %d: Sofinco data %s', $order->id, $result['data']));
    ...
    public function addOrderPayment(Order $order, $type, $params, $method)
    {
        ...
        if (Validate::isLoadedObject($cart)) {
            ...
            $db = new SofincoDb();
            if (!$db->insert('sofinco_order', $data)) {
    ...
    public function addOrderRecurringDetails(Order $order, $amountPaid)
    {
        ...
        $db = new SofincoDb();
        if (!$db->insert('sofinco_recurring', $data)) {
            $this->logError(sprintf('Unable to save order payment for order %d: %s', $order->id, $db->getMsgError()));
        }
    }
...
//             $kwixo = new SofincoKwixo($this->getConfig());
...
//         }
        ...
        $values['PBX_CUSTOMER'] = trim(substr($this->getCustomerInformation($customer),21));      
        // //Billing information
        $values['PBX_BILLING'] = trim(substr($this->getBillingInformation($customer, $address, $country, $id_lang),21));
        
        $this->logDebug(sprintf('Customer :%s ', $values['PBX_CUSTOMER']));
        $this->logDebug(sprintf('Billing :%s ', $values['PBX_BILLING']));
        // Module information
		...
		/*JC 20191003 except for limonetik */
        if ($this->isMobile() && $card['type_payment'] != "LIMONETIK") {
    ...
    public function isRecurring($orderId)
    {
        ...
        if (($details['payment_by'] == 'SofincoRecurring')) {
            return true;
        }
    ...
    public function checkUrls(array $urls)
    {
        ...
        // $client->setTimeout(5);
        // $client->setUserAgent('Sofinco');
        // $client->setFollowRedirect(false);
        // $error = null;
        // $this->logDebug(sprintf('URL to check %s', count($urls)));

        foreach ($urls as $url) {
            // $testUrl = preg_replace('#^([a-zA-Z0-9]+://[^/]+)(/.*)?$#', '\1', $url);
            // $this->logDebug(sprintf('Checking Sofinco URL %s', $testUrl));
    ...
    public function deleteRecurringPayment(Order $order, array $details)
    {
        ...
        // Check answer
        if (($result['status'] != 200) || !preg_match('#^(.*&)?ACQ=OK(&.*)$#', $result['status'])) {
        ...
        // Update details
        $sql = 'UPDATE `%ssofinco_order` SET `payment_status` = "canceled"'
            .' WHERE `id_order` = %d';
        ...
        if ($details['payment_status'] == 'capture') {
            ...
            $history->changeIdOrderState(Configuration::get('SOFINCO_STATE_MIN_CAPTURE'), $history->id_order);
    ...
    public function getOrderDetails($orderId)
    {
        ...
        $sql = 'SELECT * FROM `%ssofinco_order` WHERE `id_order` = %d';
        $sql = sprintf($sql, _DB_PREFIX_, $orderId);
    ...
    public function getOrderPaymentBy($orderId)
    {
        ...
        $sql = 'SELECT `payment_by` FROM `%ssofinco_order` WHERE `id_order` = %d';
        $sql = sprintf($sql, _DB_PREFIX_, $orderId);
        return Db::getInstance()->getValue($sql);
    }
    ...
    public function getOrderRecurringDetails($orderId)
    {
        ...
        $sql = 'SELECT * FROM `%ssofinco_recurring` WHERE `id_order` = %d';
        $sql = sprintf($sql, _DB_PREFIX_, $orderId);
        return Db::getInstance()->getRow($sql);
    }
    ...
    public function getOrderTransactionId($orderId)
    {
        ...
        $sql = 'SELECT `id_transaction` FROM `%ssofinco_order` WHERE `id_order` = %d';
        $sql = sprintf($sql, _DB_PREFIX_, $orderId);
        return Db::getInstance()->getValue($sql);
    }
    ...
    {
        $db = Db::getInstance();
        $sql = 'SELECT * FROM `%ssofinco_card` WHERE `type_card` = "%s"';
     ...
     *
     * 3.0.11 Removed sleep, now on SofincoController
     *
     * @version  3.0.11
     * @param    string $cardType
    ...
    {
        $db = Db::getInstance();
        $sql = 'SELECT * FROM `%ssofinco_card` WHERE `id_card` = %d';
    ...
    {
        $db = Db::getInstance();
        $sql = 'SELECT `type_payment`, `type_card` FROM `%ssofinco_card` WHERE `mixte` = 1';
        $sql = sprintf($sql, _DB_PREFIX_);
    ...
    {
        return (Configuration::get('SOFINCO_WEB_CASH_DIRECT') == 1 || Configuration::get('SOFINCO_WEB_CASH_DIRECT') == 2);
    }
    ...
    public function makeCaptureAll(Order $order, array $details, $changestate = true)
    {
        ...
        if (empty($response)) {
            // Call error
            $this->logError(sprintf('Order %d: Sofinco call error', $order->id));
            return 2;
        } elseif ($response['CODEREPONSE'] != '00000') {
            // Payment platform error
            $this->logError(sprintf('Order %d: Sofinco returned an error of %s', $order->id, $response['CODEREPONSE']));
        ...
        // Success
        $this->logError(sprintf('Order %d: Success of Sofinco call', $order->id));
        $message = $this->l('Capture operation:').chr(10).chr(13);
        ...
        // Mixed payment: re-adapt amount to keep same based amount in Sofinco table
        if ($isMixed) {
            $amount += $orderPayments['mixed']->amount;
        }

        // Update database
        $sql = 'UPDATE `%ssofinco_order` SET `payment_status` = "capture",'
        ...
        if (!Db::getInstance()->execute($sql)) {
            die(Tools::displayError('Error when updating Sofinco database'));
        }
        ...
        if ($partialRefund == 0) {
            ...
            $stateId = Configuration::get('SOFINCO_STATE_MIN_CAPTURE');
        }
...
/*
        // Update database
        $sql = 'UPDATE `%ssofinco_order` SET `payment_status` = "%s",'
...
*/
    ...
    public function makeRefundAmount(Order $order, array $details, $amount)
    {
        ...
        // Update database
        $sql = 'UPDATE `%ssofinco_order` SET `refund_amount` = refund_amount + %d'
            .' WHERE `id_order` = %d';
    ...
    public function signValues(array $values)
    {
        // Serialize values
        $params = array();
		// $values['PBX_CUSTOMER'] = urlencode($values['PBX_CUSTOMER']);
    ...
    {
        $db = Db::getInstance();
        $sql = 'UPDATE `%ssofinco_recurring` SET `number_term` = %d, `amount_paid` = %d WHERE `id_order` = %d';
    ...
    {
        $db = Db::getInstance();
        $sql = 'UPDATE `%ssofinco_order` SET `initial_amount` = %d, `amount` = %d WHERE `id_order` = %d';
...
*/
    ...
    public function updatePayments($order, $details, $newAmount, $operationAmount, $amountScale, $currency)
    {
        ...
        // Update Payment
        $sql = 'UPDATE `%ssofinco_order` SET `amount` = %d'
        . ' WHERE `id_order` = %d';
                    ...
                    foreach ($orderPayments as $orderPayment) {
                        if ($orderPayment->transaction_id == $details['id_transaction']) {
                            ...
                            if (!$order->addOrderPayment($operationAmount, $orderPayment->payment_method, $transactionId, $currency, null, $orderInvoice)) {
                                ...
                                if (!Db::getInstance()->execute($sql)) {
                                    $errors[] = 'The initial payment amount in Sofinco could not be reset';
                                }
                                return array(
                                    'status' => 0,
                                    'error' => $errors,
    ...
    {
        $displayMode = Configuration::get('SOFINCO_PAYMENT_DISPLAY', 0);
        $name = $moduleName;
    ...
    public function hasCartLocker($cartId, $transactionId)
    {
        ...
        $sql = 'SELECT COUNT(*) FROM `%ssofinco_cart_locker` WHERE `id_cart` = %d AND `id_transaction` = "%s"';
    ...
    public function createCartLocker($cartId, $transactionId)
    {
        ...
        $db = new SofincoDb();
        if (!$db->insert('sofinco_cart_locker', $data)) {
    ...
    public function getCustomerInformation(Customer $customer)
    {
        $id = $customer->id;
        
//         $xmlCustomerInformation = array();
//         $xmlCustomerInformation['Customer'][] = array(
//             'Id' => $id
//         );          
    ...
    public function getBillingInformation(Customer $customer, Address $address, $country, $id_lang)
    {
     ...
     * @return Country
     */
    public static function getCountryById($idCountry)
    {
    ...
    /**
     * Return the country numeric iso code corresponding to the iso_code parameter
     * @param int $idCountry
     * @return Country
     */
    public static function getCountryNumericIsoCode($countryIsoCode)
    {
        $_countryCodes =
        '[
        ...
        $countryCodes = json_decode($_countryCodes);
        foreach ($countryCodes as $countryCode) {
            if ($countryCode->iso_code == $countryIsoCode) {
                return $countryCode->numeric_iso_code;
            }
        }
        return '';
    }
