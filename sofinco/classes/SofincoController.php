 ...
 */
class SofincoController extends SofincoAbstract
{

    public function __construct()
    {
        parent::__construct(new Sofinco());
    }
    ...
    public function cancelAction()
    {
        try {
            ...
            if ($params !== false) {
                ...
                $message = sprintf('Cart %d: Payment was canceled by user on Sofinco payment page.', $cart->id);
    ...
    public function failureAction()
    {
        try {
            ...
            if ($params !== false) {
                $cart = $this->getHelper()->untokenizeCart($params['reference']);
                $message = sprintf('Cart %d: Customer is back from Sofinco payment page. Payment refused by Sofinco (%d).', $cart->id, $params['error']);
                $this->logDebug($message);
            ...
            */
            ...
            foreach ($requiredParams as $requiredParam) {
                if (!isset($params[$requiredParam])) {
                    $message = sprintf($this->l('Missing %s parameter in Sofinco call'), $requiredParam);
                    $this->logFatal(sprintf('Cart %d: (IPN) %s', $cart->id, $message));
                    // TODO no way to associate this message to an order...
                    throw new Exception($message);
                }
            ...
            if (in_array($params['error'], array('00000', '00200', '00201', '00300', '00301', '00302', '00303'))) {
                ...
                // Payment refused
                $message = sprintf('Cart %d: (IPN) Payment was refused by Sofinco (%d).', $cart->id, $params['error']);
                $this->logDebug($message);
                // TODO no way to associate this message to an order...
            }
        } catch (Exception $e) {
    ...
    public function redirectAction()
    {
        ...
        <html>
            <body>
                ...
                <form action="<?php echo Tools::htmlentitiesUTF8($url); ?>" method="post" name="Sofinco" enctype="application/x-www-form-urlencoded">
                    <p><center>
                        <?php
                        if ($debug) {
        ...
        <?php
        if (!$debug) {
            echo '<script>document.forms["Sofinco"].submit();</script>';
        }
        ?>
        </body>
        </html>
        <?php
        $message = sprintf('Cart %d: Redirecting customer to Sofinco (%s, %s).', $cart->id, $values['PBX_TYPEPAIEMENT'], $values['PBX_TYPECARTE']);
        $this->logDebug($message);
    }
