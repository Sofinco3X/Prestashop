...
class Mobile_Validator {
	...
	function normalize( $phone_number, $country_name = null, $onlynumber = false ) {
		...
		if( $country_name ) {
			...
			if ( $is_plus_prefixed ) {
				...
				if( in_array( strlen( $phone_number ), $iso3166_entry["phone_number_lengths"] ) ) {
					if( !$onlynumber ) 	$phone_number = $iso3166_entry["country_code"] . $phone_number;
				}
			}
		} else {
			if ( $is_plus_prefixed ) {
				...
				if ( in_array( strlen( $phone_number ), $iso3166_entry["phone_number_lengths"] ) ) {
					if( !$onlynumber ) 	$phone_number = "1" . $phone_number;
				}
			}
		}
