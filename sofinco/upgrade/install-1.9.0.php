...
function upgrade_module_1_9_0($object)
{
    ...
    $result = Db::getInstance()->getRow('SELECT *
        ...
        WHERE TABLE_SCHEMA = "'._DB_NAME_.'" AND TABLE_NAME = "'._DB_PREFIX_.'sofinco_card" AND COLUMN_NAME = "'.$column.'"');
    if (false == $result) {
        if (Db::getInstance()->execute('ALTER TABLE `'._DB_PREFIX_.'sofinco_card` ADD `3ds` int(1) NULL')) {
            $cards = array();
            ...
            foreach ($cards as $card) {
                $sql = 'UPDATE `'._DB_PREFIX_.'sofinco_card` SET `'.$column.'` = '.(int)$card[$column].' WHERE `type_card` = "'.$card['card'].'"';
