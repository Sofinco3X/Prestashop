...
class SofincoKwixo
{
    ...
    public function __construct(SofincoConfig $config)
    {
        $this->_config = $config;
    }
    ...
    public function buildKwixoParams(Cart $cart, array $additionalParams = array())
    {
        ...
        $deliveryDelay = intval(Configuration::get('SOFINCO_CARRIER_DAYS_'.$carrier->id));
        ...
        // Delivery Speed
        $deliverySpeed = Configuration::get('SOFINCO_CARRIER_TYPE_'.$carrier->id);
        if (!in_array($deliverySpeed, array(1, 2))) {
            $deliverySpeed = 1;
        }

        // Delivery type
        $deliveryType = Configuration::get('SOFINCO_CARRIER_TYPE_'.$carrier->id);
        ...
        $default = Configuration::get('SOFINCO_DEFAULTCATEGORYID');
        if (empty($default)) {
            $default = 1;
        }
        foreach ($products as $product) {
            ...
            foreach ($categories as $category) {
                $value = Configuration::get('SOFINCO_CAT_TYPE_'.$category['id_category']);
                if ($value) {
                    $selected = Configuration::get('SOFINCO_CAT_TYPE_'.$category['id_category']);
                    break;
                }
            }
