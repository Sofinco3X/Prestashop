...
require_once dirname(__FILE__).'/SofincoHtmlWriterAbstract.php';

/**
 * HTML write for PrestaShop 1.4/1.5
 */
class SofincoHtmlWriter extends SofincoHtmlWriterAbstract
{
    public function _alert($type, $content, $id, $show)
    {
        ...
        $tpl = '<div class="alert alert-sofinco %s"%s>%s</div>';
        $content = $content;
    ...
    {
        $tpl = '<br/><fieldset id="%s"><legend>%s</legend><div class="etran_block_content">';
        if (!empty($image)) {
