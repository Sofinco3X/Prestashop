...
class SofincoInstaller
{
    ...
    {
        if (!$this->installSofincoTables($module)) {
            return array(
                'status' => false,
        ...
        if (!$this->installSofincoOrderStates($module)) {
            return array(
                'status' => false,
        ...
        if (!$this->installSofincoCard($module)) {
            return array(
                'status' => false,
     ...
     * @return boolean
     */
    public function installSofincoTables($module)
    {
        // Order table
        $sql = 'CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'sofinco_order` (
        ...
        if (!Db::getInstance()->execute($sql)
            ...
            $module->logFatal('Table creation failed for "sofinco_order"');
            $module->logFatal('   SQL: '.$sql);
            return false;
        }

        // Check if column 'method' exists and create it if not
        if (!$this->_sqlColumnExists(_DB_PREFIX_.'sofinco_order', 'method')) {
            Db::getInstance()->execute('ALTER TABLE `'._DB_PREFIX_.'sofinco_order` ADD `method` varchar(30) NULL');
        }
        // Check if column 'carte_num' exists and create it if not
        if (!$this->_sqlColumnExists(_DB_PREFIX_.'sofinco_order', 'carte_num')) {
            Db::getInstance()->execute('ALTER TABLE `'._DB_PREFIX_.'sofinco_order` ADD `carte_num` varchar(30) NULL');
        }
        // Check if column 'initial_amount' exists and create it if not
        if (!$this->_sqlColumnExists(_DB_PREFIX_.'sofinco_order', 'initial_amount')) {
            Db::getInstance()->execute('ALTER TABLE `'._DB_PREFIX_.'sofinco_order` ADD `initial_amount` int(10) unsigned NOT NULL');
        }

        // Order Recurring table
        $sql = 'CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'sofinco_recurring` (
            `id_sofinco_recurring` INT(10) NOT NULL AUTO_INCREMENT ,
            `id_order` INT(10) NOT NULL ,
            `status` VARCHAR(255) NOT NULL ,
            `number_term` INT(1) NOT NULL ,
            `amount_paid` INT(15) NOT NULL ,
            PRIMARY KEY (`id_sofinco_recurring`))
            ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=utf8';
        if (!Db::getInstance()->execute($sql)
        ) {
            $module->logFatal('Table creation failed for "sofinco_recurring"');
            $module->logFatal('   SQL: '.$sql);
            return false;
        }

        // CartLocker table
        $sql = 'CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'sofinco_cart_locker` (
        ...
        if (!Db::getInstance()->execute($sql)
            ...
            $module->logFatal('Table creation failed for "sofinco_cart_locker"');
            $module->logFatal('   SQL: '.$sql);
            return false;
        }
     ...
     * @return boolean
     */
    public function installSofincoOrderStates($module)
    {
        if (!$this->_isValidState(Configuration::get('SOFINCO_ID_ORDER_STATE_NX'))) {
            $orderState = new OrderState();
            $orderState->name = array();
            foreach (Language::getLanguages() as $language) {
                if (strtolower($language['iso_code']) == 'fr') {
                    $orderState->name[$language['id_lang']] = 'Payé partiellement via Sofinco';
                } else {
                    $orderState->name[$language['id_lang']] = 'Partially paid through Sofinco';
                }
            }
            ...
            if ($orderState->add()) {
                ...
                $module->logFatal('Order State creation failed for '.Configuration::get('SOFINCO_ID_ORDER_STATE_NX'));
            }
            Configuration::updateValue('SOFINCO_ID_ORDER_STATE_NX', (int)$orderState->id);
        }
        Configuration::updateValue('SOFINCO_MIDDLE_STATE_NX', Configuration::get('SOFINCO_ID_ORDER_STATE_NX'));

        if (!$this->_isValidState(Configuration::get('SOFINCO_RECEIVE_PAY'))) {
            ...
            if ($orderState->add()) {
                ...
                $module->logFatal('Order State creation failed for '.Configuration::get('SOFINCO_RECEIVE_PAY'));
            }
            Configuration::updateValue('SOFINCO_RECEIVE_PAY', (int)$orderState->id);
        }

        if (!$this->_isValidState(Configuration::get('SOFINCO_STATE_MIN_CAPTURE'))) {
            $orderState = new OrderState();
            $orderState->name = array();
            foreach (Language::getLanguages() as $language) {
                if (strtolower($language['iso_code']) == 'fr') {
            ...
            if ($orderState->add()) {
                ...
                $module->logFatal('Order State creation failed for '.Configuration::get('SOFINCO_STATE_MIN_CAPTURE'));
            }
            Configuration::updateValue('SOFINCO_STATE_MIN_CAPTURE', (int)$orderState->id);
        }

        if (!$this->_isValidState(Configuration::get('SOFINCO_WEB_CASH_VALIDATION'))) {
            // Capture manuelle
            $orderState = new OrderState();
            $orderState->name = array();
            foreach (Language::getLanguages() as $language) {
            ...
            if ($orderState->add()) {
                ...
                $module->logFatal('Order State creation failed for '.Configuration::get('SOFINCO_WEB_CASH_VALIDATION'));
            }
            Configuration::updateValue('SOFINCO_WEB_CASH_VALIDATION', (int)$orderState->id);
        }
/*
        if (!$this->_isValidState(Configuration::get('SOFINCO_KWIXO'))) {
            $orderState = new OrderState();
            $orderState->name = array();
            foreach (Language::getLanguages() as $language) {
                if (strtolower($language['iso_code']) == 'fr') {
            ...
            Configuration::updateValue('SOFINCO_KWIXO', (int)$orderState->id);
        }
*/
        return true;
    }
     ...
     * @return boolean
     */
    public function installSofincoCard($module)
    {
        $db = new SofincoDb();

        $sql = 'CREATE TABLE IF NOT EXISTS `%ssofinco_card` (
        ...
        if (!$result) {
            $module->logFatal('Table creation failed for "sofinco_card"');
            $module->logFatal('   SQL: '.$sql);
            return false;
        }
        
        // Sofinco 3XCB & 3XCB no charge
        $SofincoPayment = array(
            'type_payment' => 'LIMONETIK',
            'type_card' => '3XCB',
            'label' => '2x,3x ou 4x par carte bancaire',
            'active' => 1,
            'debit_expedition' => 0,
            'debit_immediat' => 1,
            'debit_differe' => 0,
        ...
        foreach ($cards as $card) {
            if (!$db->insert('sofinco_card', $card)) {
                $module->logFatal('Table data creation failed for "sofinco_card" '.$card['label']);
                return false;
            }
        }
    ...
    public function uninstall($module)
    {
        ...
        // $idOrderState = Configuration::get('SOFINCO_ID_ORDER_STATE_NX');
        // if($idOrderState != '') {
        ...
        // $idOrderState = Configuration::get('SOFINCO_RECEIVE_PAY');
        // if($idOrderState != '') {
        //     Db::getInstance()->execute('DELETE FROM `'._DB_PREFIX_.'order_state` WHERE id_order_state='.$idOrderState);
        //     Db::getInstance()->execute('DELETE FROM `'._DB_PREFIX_.'order_state_lang` WHERE id_order_state='.$idOrderState);
        //     @unlink(_PS_IMG_DIR_.'os/'.$idOrderState.'.gif');
        ...
        // $idOrderState = Configuration::get('SOFINCO_STATE_MIN_CAPTURE');
        // if($idOrderState != '') {
        //     Db::getInstance()->execute('DELETE FROM `'._DB_PREFIX_.'order_state` WHERE id_order_state='.$idOrderState);
        //     Db::getInstance()->execute('DELETE FROM `'._DB_PREFIX_.'order_state_lang` WHERE id_order_state='.$idOrderState);
        //     @unlink(_PS_IMG_DIR_.'os/'.$idOrderState.'.gif');
        ...
        // $idOrderState = Configuration::get('SOFINCO_KWIXO');
        // if($idOrderState != '') {
        //     Db::getInstance()->execute('DELETE FROM `'._DB_PREFIX_.'order_state` WHERE id_order_state='.$idOrderState);
        //     Db::getInstance()->execute('DELETE FROM `'._DB_PREFIX_.'order_state_lang` WHERE id_order_state='.$idOrderState);
        //     @unlink(_PS_IMG_DIR_.'os/'.$idOrderState.'.gif');
        ...
        // - sofinco_recurring
        // - sofinco_order
        $tables = array('sofinco_card', 'sofinco', 'sofinco_cart_locker');
        ...
        // Configuration::deleteByName('SOFINCO_ID_ORDER_STATE_NX');
        // Configuration::deleteByName('SOFINCO_RECEIVE_PAY');
        // Configuration::deleteByName('SOFINCO_STATE_MIN_CAPTURE');
    ...
    {
        if ($action) {
            $sql = 'SHOW TABLES LIKE "'._DB_PREFIX_.'SOFINCO_order" ';
            $result = DB::getInstance()->executeS($sql);

            if (false != $result && 0 < count($result)) {
                // Migrate DB data for previous orders
                $sql = sprintf('INSERT IGNORE INTO %ssofinco_order (`id_order`, `id_transaction`, `num_appel`, `ref_abo`, `payment_status`, `amount`, `initial_amount`, `currency`, `payment_by`, `carte`, `pays`, `ip`, `secure`, `date`, `refund_amount`)
                    SELECT t.`id_order`, t.`id_transaction`, t.`num_appel`, t.`ref_abo`, t.`payment_status`, t.`amount`, t.`amount`, t.`currency`, t.`payment_by`, t.`carte`, t.`pays`, t.`ip`, t.`secure`, t.`date`, t.`refund_amount` FROM %sSOFINCO_order t', _DB_PREFIX_, _DB_PREFIX_);

                if (Db::getInstance()->execute($sql)) {
                    $sql = sprintf('DROP TABLE IF EXISTS `%sSOFINCO_order`;', _DB_PREFIX_);
                    DB::getInstance()->execute($sql);
                }
                ...
                if (file_exists($path)) {
                    ...
                    if (!preg_match('/\# Sofinco start(.*?)\# Sofinco end/s', $content, $m)) {
                        $append = "\n# Sofinco start\nRewriteEngine on\nRewriteBase /\nRewriteRule modules/SofincoEpayment/$ modules/sofinco/ [QSA,L,R=301]\n# Sofinco end\n\n";
            ...
            if (file_exists($path)) {
                ...
                if ($htaccess = fopen($path, 'w')) {
                    if (preg_match('/\# Sofinco start(.*?)\# Sofinco end/s', $content, $m)) {
                        $positionStart = strpos($content, '# Sofinco start');
                        $positionEnd = strpos($content, '# Sofinco end');
                        if (false !== $positionStart && false != $positionEnd) {
                            $contentNew = Tools::substr($content, 0, ($positionStart - 1));
                            $contentNew .= Tools::substr($content, $positionEnd + Tools::strlen('# Sofinco end'));
